...
func CreateSSLCommerzSession(ctx context.Context, req SSLCommerzSessionRequest) (SSLCommerzSessionResponse, error) {
	...
	if response.GatewayPageURL == "" {
		...
		return SSLCommerzSessionResponse{}, fmt.Errorf("%s", reason)
	}
...
func GetSSLCommerzPaymentURL(data map[string]interface{}) string {
	if paymentURL, ok := data["payment_url"].(string); ok && paymentURL != "" {
		return paymentURL
	}
	if gatewayURL, ok := data["gateway_url"].(string); ok && gatewayURL != "" {
		return gatewayURL
	}
	if config.SSLCommerz != nil && config.SSLCommerz.FrontendURL != "" {
		return config.SSLCommerz.FrontendURL + "/payment/result"
	}
	return "http://localhost:3000/payment/result"
}

func SSLCommerzSuccess(c *gin.Context) {
	c.String(http.StatusOK, "SSLCommerz payment success callback received")
}

func SSLCommerzFail(c *gin.Context) {
	c.String(http.StatusOK, "SSLCommerz payment failed callback received")
}

func SSLCommerzCancel(c *gin.Context) {
	c.String(http.StatusOK, "SSLCommerz payment cancelled callback received")
}

func SSLCommerzIPN(c *gin.Context) {
	c.String(http.StatusOK, "SSLCommerz IPN received")
}
