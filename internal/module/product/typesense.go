...
func EnsureTypesenseProductsCollection(ctx context.Context) error {
	...
	_, status, err := config.Typesense.Request(ctx, http.MethodGet, path, nil, nil) // next time
	if err == nil {
		return nil
	}
	...
	_, _, err = config.Typesense.Request(ctx, http.MethodPost, "/collections", nil, schema) // from here
	return err
}

func UpsertProductInTypesense(ctx context.Context, product Product) error {
	if config.Typesense == nil {
		return nil
	}
...
func SearchProductsWithTypesense(ctx context.Context, params GetProductsParams) ([]ProductResponse, int64, bool, error) {
	if config.Typesense == nil {
		return nil, 0, false, fmt.Errorf("typesense is not enabled")
	}
	...
	query.Set("query_by", "name") // full text search
	...
	products := make([]ProductResponse, 0, len(response.Hits))
	...
	return products, int64(response.Found), hasNext, nil
}
...
func typesenseProductDocumentFromModel(product Product) typesenseProductDocument {
...
func BulkUpsertProductsToTypesense(ctx context.Context, products []Product) error {
